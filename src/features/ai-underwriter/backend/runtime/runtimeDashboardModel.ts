import { runV2RuntimeSmokeTest } from './runtimeSmokeTest';
...
export type RuntimeDashboardModel = {
  ...
  smokeTest: Awaited<ReturnType<typeof runV2RuntimeSmokeTest>>;
};

export async function loadRuntimeDashboardModel(): Promise<RuntimeDashboardModel> {
  const smokeTest = await runV2RuntimeSmokeTest();

  return {
    title: 'V2 Backend Brain Runtime Console',
    subtitle: 'Runtime-mounted V1 decision, V2 audit, snapshot, and governance packet flow.',
    smokeTest,
    statusCards: [
      ...
      { label: 'Governance', value: `${smokeTest.governanceStatus}`, detail: 'Packet route response', tone: 'cyan' }
    ],
    routeContracts: [
